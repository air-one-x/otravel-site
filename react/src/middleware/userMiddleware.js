...
export default (store) => (next) => (action) => {
  switch (action.type) {
    case LOGIN:
      ...
      axios({
        method: 'post',
        url: 'http://ec2-3-86-88-131.compute-1.amazonaws.com/login',
        ...
        axios({
          url: 'http://ec2-3-86-88-131.compute-1.amazonaws.com/isLogged',
          method: 'post',
          withCredentials: true,
          headers: {
      ...
      case LOGOUT:
        axios({
          method: 'post',
          url: 'http://ec2-3-86-88-131.compute-1.amazonaws.com/logout',
          withCredentials: true,
        })
          .then((res) => {
          // Si succès -> dispatcher une action success
        ...
        case CHECK_AUTH:
          ...
          case INSERT_NEW_INFORMATION:
            ...
            axios({
              method: 'post',
              url: `http://ec2-3-86-88-131.compute-1.amazonaws.com/user/edit/${id}`,
              withCredentials: true,
              headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${localStorage.getItem('id_token')}`,
